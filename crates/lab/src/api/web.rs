...
fn embedded_web_file(relative: &str) -> Option<&'static [u8]> {
    EMBEDDED_WEB_FILES
        .iter()
...
pub fn embedded_web_assets_available() -> bool {
    embedded_web_file("index.html").is_some()
}
...
fn resolve_embedded_asset(request_path: &str) -> Option<(&'static [u8], PathBuf)> {
    ...
    if let Some(bytes) = embedded_web_file(&embedded_lookup_key(&relative)) {
        return Some((bytes, relative));
    }

    let index_path = relative.join("index.html");
    if let Some(bytes) = embedded_web_file(&embedded_lookup_key(&index_path)) {
        return Some((bytes, index_path));
    }

    embedded_web_file("index.html").map(|bytes| (bytes, PathBuf::from("index.html")))
}
