...
//! surface-neutral — no surface module should own it, and `From<ServiceError>`
//! conversions whose source types live in `lab-apis` must be defined alongside
//! the local `ToolError` type to satisfy Rust's orphan rule.
...
//
// All SDK error → ToolError conversions whose source error type lives in
// `lab-apis` live here so that the orphan rule is satisfied (both the local
// `ToolError` and these impls are owned by this crate). Conversions from error
// types local to the `lab` crate stay in `lab` (e.g. `RegistryStoreError`).
// Post gateway-pivot the per-service macro is gone; remaining impls are
// hand-written for the surviving feature-gated SDK error types.
...
mod tests {
    ...
    fn sdk_variant_promotes_sdk_kind_to_top_level_kind() {
        ...
        let s = serde_json::to_string(&e).expect("ToolError is always serializable");
        assert!(
            s.contains("\"kind\":\"rate_limited\""),
            "Sdk variant must promote sdk_kind to a top-level `kind` field, got: {s}"
        );
        assert!(
            !s.contains("\"sdk_kind\""),
            "sdk_kind must not appear as a separate field, got: {s}"
        );
