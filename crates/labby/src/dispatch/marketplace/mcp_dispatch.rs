...
use labby_apis::mcpregistry::types::{EnvironmentVariable, ServerJSON};
...
fn store_list_params_from_mcp_params(
    params: &labby_apis::mcpregistry::types::ListServersParams,
...
fn build_stdio_command<'a>(
    pkg: &'a labby_apis::mcpregistry::types::Package,
    server_name: &str,
...
async fn mcp_client_config(
    ...
    if let Some(url) = server.remotes.iter().find_map(|r| r.url.as_deref()) {
        labby_runtime::security::ssrf::validate_external_https_url(url).await?;
