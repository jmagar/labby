...
use labby_apis::stash::{MarketplaceOrigin, StashComponentKind, StashOrigin};
use serde::Serialize;
...
async fn normalize_marketplace_workspace(
    ...
    save_label: &str,
) -> Result<Option<labby_apis::stash::StashRevision>, ToolError> {
    let Some(path) = artifact_path else {
        return Ok(None);
    };
    let store_for_blocking = store.clone();
...
mod tests {
    use super::*;
    use labby_apis::stash::{StashComponent, StashWorkspaceShape};
    use serde_json::Value;
