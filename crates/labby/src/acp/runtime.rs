...
struct RuntimeStarted {
    ...
    models: Vec<labby_apis::acp::types::AcpModelOption>,
    config_options: Vec<labby_apis::acp::types::AcpSessionConfigOptionView>,
}
...
fn session_config_options(
    ...
    Option<String>,
    Vec<labby_apis::acp::types::AcpModelOption>,
    Vec<labby_apis::acp::types::AcpSessionConfigOptionView>,
) {
    use labby_apis::acp::types::{AcpModelOption, AcpSessionConfigOptionView};
    ...
    drop(
        event_tx
            .send(session_state_event(
                ...
                provider_id,
                labby_apis::acp::types::AcpSessionState::Running,
            ))
            .await,
    );
        ...
        let update = match update {
            ...
            PromptTurnMessage::Stop(Ok(stop_reason)) => {
                ...
                let state = if stop_reason == "cancelled" {
                    labby_apis::acp::types::AcpSessionState::Cancelled
                } else {
                    labby_apis::acp::types::AcpSessionState::Completed
                };
                drop(
                    event_tx
                        .send(session_state_event(
                    ...
                    event_tx
                        .send(provider_info_event(
                            ...
                            json!({
                                ...
                                "status": match state {
                                    labby_apis::acp::types::AcpSessionState::Cancelled => "cancelled",
                                    _ => "completed",
                                },
                ...
                drop(
                    event_tx
                        .send(session_state_event(
                            session_id.to_string(),
                            provider_id,
                            labby_apis::acp::types::AcpSessionState::Failed,
                        ))
                        .await,
                );
                drop(
...
fn session_state_event(
    session_id: String,
    provider: &str,
    state: labby_apis::acp::types::AcpSessionState,
) -> AcpEvent {
...
fn unfinished_prompt_exit_event(
...
) -> (labby_apis::acp::types::AcpSessionState, AcpEvent) {
    if let Some(error) = run_error {
        return (
            labby_apis::acp::types::AcpSessionState::Failed,
            provider_info_event(
    ...
    let status = match state {
        labby_apis::acp::types::AcpSessionState::Completed => "completed",
        _ => "failed",
    };
    (
        state,
...
mod tests {
    ...
    fn unfinished_prompt_with_provider_error_fails_even_after_progress() {
        ...
        assert_eq!(state, labby_apis::acp::types::AcpSessionState::Failed);
    ...
    fn unfinished_prompt_without_provider_error_preserves_idle_completion_heuristic() {
        ...
        assert_eq!(state, labby_apis::acp::types::AcpSessionState::Completed);
        let AcpEvent::ProviderInfo { raw, .. } = event else {
            panic!("expected provider_info event");
        };
