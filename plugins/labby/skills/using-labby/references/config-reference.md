...
`gateway.code_mode.set` currently updates only `timeout_ms`,
`max_tool_calls`, `max_response_bytes`, and `max_response_tokens`; the other
fields must be edited in `config.toml` unless the API surface has changed.
